 ...
 * amount of method inside the {@link com.ensi.pcd.ourdeal.AdhocActivity}
 * <p></p>
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION.equals(action)) {
            ...
            if (networkInfo.isConnected()) {
                ...
                ((AdhocActivity)activity).setConnected(true);

                //now color the active tabs
                ((AdhocActivity)activity).addColorActiveTabs(false);

                //and change the visible tab
                ((AdhocActivity)activity).setTabFragmentToPage(((AdhocActivity)activity).getTabNum());
                ...
                ((AdhocActivity)activity).addColorActiveTabs(true);

                //if manualItemMenuDisconnectAndStartDiscovery() is not activated by the user
                if(!((AdhocActivity)activity).isBlockForcedDiscoveryInBroadcastReceiver()) {

                    //force stop discovery process
                    ((AdhocActivity) activity).forceDiscoveryStop();
                    ((AdhocActivity) activity).restartDiscovery();
                }

                //disable all chatmanagers
                ((AdhocActivity)activity).setDisableAllChatManagers();

                //change the visible tab to the first one, because i want to see the available services to reconnect, in necessary
                ((AdhocActivity)activity).setTabFragmentToPage(0);

                ((AdhocActivity)activity).setConnected(false);
